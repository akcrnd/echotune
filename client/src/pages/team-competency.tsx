    ...
    <div className="flex items-center justify-center gap-1">
      ...
      <div className="flex flex-col gap-0.5 ml-1">
        <button
          ...
          className="px-2 py-0.5 text-sm font-bold bg-white hover:bg-slate-50 border border-slate-200 rounded shadow-sm cursor-pointer disabled:opacity-40"
        >
          +0.5
        </button>
        <button
...
export default function TeamCompetency() {
  ...
  const continuousCompetencyGroups = useMemo(() => {
    if (!detail) return competencyGroups;

    const matchedKeys = new Set<string>();
    const categoryGroups = detail.workCategories.map((category, index) => {
      const categoryNoKey = normalizeKey(category.categoryNo);
      const categoryNameKey = normalizeKey(category.categoryName);
      const matchedGroup = competencyGroups.find(
        (group) =>
          normalizeKey(group.majorNo) === categoryNoKey ||
          normalizeKey(group.majorName) === categoryNameKey ||
          group.key === categoryNoKey ||
          group.key === categoryNameKey,
      );

      if (matchedGroup) {
        matchedKeys.add(matchedGroup.key);
      }

      return {
        key: normalizeKey(category.categoryNo ?? category.categoryName ?? index),
        majorNo: category.categoryNo,
        majorName: category.categoryName,
        workCategory: category,
        workCategoryIndex: index,
        rows: matchedGroup?.rows ?? [],
      };
    });

    const orphanGroups = competencyGroups.filter((group) => !matchedKeys.has(group.key));
    return [...categoryGroups, ...orphanGroups];
  }, [competencyGroups, detail]);
      ...
      <div className="relative w-[300px] rounded-2xl border border-slate-200 bg-gradient-to-br from-white to-slate-50/50 p-4 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] hover:-translate-y-0.5 transition-all duration-300">
        ...
        <div className="pl-2 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="outline" className="text-sm text-muted-foreground border-slate-200 bg-white/80">
              증빙 {evidenceCount}건
            </Badge>
          </div>
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-sm text-slate-400">{member?.employeeNumber ?? "조직도 미연동"}</span>
          </div>

          <div className="space-y-1 bg-white/60 dark:bg-slate-900/40 p-2.5 rounded-lg border border-slate-100/80">
            <span className="text-sm font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider">담당 직무</span>
            {isEditMode ? (
              <Input
                value={row.roleTitle ?? ""}
                onChange={(event) => updateArrayRow<Assignment>("assignments", rowIndex, { roleTitle: event.target.value })}
                placeholder="담당직무 입력"
                className="h-8 mt-1 border-slate-200 text-sm shadow-none focus-visible:ring-1 bg-white"
              />
            ) : (
              <p className="text-sm font-semibold text-slate-700">{row.roleTitle || "직무 미입력"}</p>
            )}
          </div>

          <div className="space-y-1">
            <span className="text-sm font-bold text-muted-foreground uppercase tracking-wider">상세 담당 업무</span>
            {isEditMode ? (
              <Textarea
                ...
                placeholder={"1. 업무내용\n2. 업무내용"}
                className="min-h-[90px] mt-1 resize-y border-slate-200 text-sm leading-relaxed bg-white"
              />
            ) : (
              <p className="text-sm text-slate-600 leading-relaxed whitespace-pre-wrap min-h-[60px] bg-slate-50/50 p-2 rounded border border-slate-100/50">
          ...
          <div className="flex items-center justify-between border-t border-slate-100 pt-2.5 text-sm">
            ...
            {isEditMode ? (
              <Input
                ...
                className="h-7 w-32 border-slate-200 text-sm shadow-none focus-visible:ring-1 bg-white"
              />
            ) : (
  ...
  return (
    <div className="p-6 space-y-6 text-[15px]" data-testid="team-competency-page">
      {/* Team and report controls */}
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-end bg-white/50 backdrop-blur-md px-4 py-3 rounded-2xl border border-slate-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
        <div className="flex flex-wrap items-center justify-end gap-2.5">
          <div className="flex items-center gap-1.5 bg-slate-100 p-1 rounded-lg border border-slate-200/50">
            <select
              ...
              className="h-10 min-w-[180px] rounded-md border-0 bg-transparent px-3 text-sm font-semibold focus-visible:ring-0 cursor-pointer"
            ...
            <Input
              ...
              className="w-24 h-10 border-0 bg-white shadow-none text-sm font-bold text-center"
          ...
          <Button
            ...
            className="bg-indigo-50 text-indigo-600 hover:bg-indigo-100 border border-indigo-200/60 shadow-none font-bold text-sm h-10 px-4"
          >
          ...
          <Button
            ...
            className="bg-primary hover:bg-primary/95 text-white font-bold text-sm h-10 px-4 shadow-sm"
          >
          ...
          {detail && (
            <div className="flex items-center gap-1.5 ml-2 border-l pl-3 border-slate-200">
              <Button
                ...
                className={`h-10 px-4 font-bold text-sm transition-all ${
                  isEditMode
                    ? "bg-amber-500 hover:bg-amber-600 border-amber-600 text-white"
                    : "text-slate-600 border-slate-200 hover:bg-slate-50"
                }`}
              >
                <Settings className={`h-4 w-4 mr-1.5 ${isEditMode ? "animate-spin" : ""}`} />
      ...
      {detail ? (
        <>
          {isReportLocked && (
            <div className="rounded-xl border border-emerald-200 bg-emerald-50/50 px-4 py-3 text-sm text-emerald-800 flex items-center gap-2">
            ...
            <div className="xl:col-span-2 grid grid-cols-2 sm:grid-cols-4 gap-4">
              <Card className="border-slate-200/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)] hover:shadow-md transition-all duration-300">
                <CardContent className="p-4 flex items-center justify-between">
                  <div className="space-y-1">
                    <p className="text-sm font-bold text-slate-400 uppercase tracking-wider">팀원 수</p>
                    <p className="text-2xl font-black text-slate-800">{detail.teamMembers.length}</p>
                    <p className="text-sm text-slate-400">현재 활성 멤버</p>
                  </div>
              ...
              <Card className="border-slate-200/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)] hover:shadow-md transition-all duration-300">
                <CardContent className="p-4 flex items-center justify-between">
                  <div className="space-y-1">
                    <p className="text-sm font-bold text-slate-400 uppercase tracking-wider">업무분류</p>
                    <p className="text-2xl font-black text-slate-800">{detail.workCategories.length}</p>
                    <p className="text-sm text-slate-400">관리 대분류 영역</p>
                  </div>
              ...
              <Card className="border-slate-200/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)] hover:shadow-md transition-all duration-300">
                <CardContent className="p-4 flex items-center justify-between">
                  <div className="space-y-1">
                    <p className="text-sm font-bold text-slate-400 uppercase tracking-wider">평가율</p>
                    <div className="flex items-baseline gap-1">
                      <p className="text-2xl font-black text-slate-800">
                        {percentValue(scoreStats.entered, scoreStats.total)}%
                      </p>
                      <span className="text-sm text-slate-400">({scoreStats.entered}/{scoreStats.total})</span>
                    </div>
                    <p className="text-sm text-emerald-600 font-semibold">평균 {scoreStats.average.toFixed(1)}점</p>
                  </div>
              ...
              <Card className="border-slate-200/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)] hover:shadow-md transition-all duration-300">
                <CardContent className="p-4 flex items-center justify-between">
                  <div className="space-y-1">
                    <p className="text-sm font-bold text-slate-400 uppercase tracking-wider">자격/증빙</p>
                    <p className="text-2xl font-black text-slate-800">{detail.qualifications.length}</p>
                    <p className="text-sm text-violet-600 font-semibold">증빙 데이터 {evidenceTotal}건</p>
                  </div>
            ...
            <div className="xl:col-span-1">
              <Card className="overflow-hidden border-slate-200/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
                <div className="bg-slate-50/50 px-4 py-2.5 border-b border-slate-100 flex items-center justify-between">
                  <span className="text-sm font-bold text-slate-500 uppercase tracking-wider">문서 승인 결재선</span>
                  <Badge variant={
                ...
                <CardContent className="p-3 grid grid-cols-5 gap-2">
                  <div className="flex flex-col items-center justify-center p-2 bg-slate-50/70 rounded-xl border border-slate-100 text-center">
                    <span className="text-sm font-bold text-slate-400">Rev</span>
                    {isEditMode ? (
                      <Input
                        ...
                        placeholder="Rev.00"
                        className="mt-1 h-6 text-center text-sm w-full bg-white px-1 py-0.5 rounded border border-slate-200"
                      />
                    ) : (
                      <span className="mt-1 text-sm font-bold text-slate-700">{detail.report.revision || "Rev.00"}</span>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col items-center justify-center p-2 bg-slate-50/70 rounded-xl border border-slate-100 text-center relative overflow-hidden">
                    <span className="text-sm font-bold text-slate-400">작성</span>
                    {isEditMode ? (
                      <Input
                        disabled={isReportLocked}
                      ...
                      <div className="mt-1 flex flex-col items-center">
                        <span className="text-sm font-bold text-slate-700 truncate max-w-full">{detail.report.preparedBy || "-"}</span>
                        {detail.report.preparedBy && (
                          <div className="absolute right-0.5 bottom-0.5 text-sm border border-red-200 text-red-500 rounded px-0.5 rotate-12 bg-white/90 font-serif font-black scale-90">
                            작성
                          </div>
                        )}
                      </div>
                  ...
                  <div className="flex flex-col items-center justify-center p-2 bg-slate-50/70 rounded-xl border border-slate-100 text-center relative overflow-hidden">
                    <span className="text-sm font-bold text-slate-400">검토</span>
                    {isEditMode ? (
                      <Input
                        disabled={isReportLocked}
                      ...
                      <div className="mt-1 flex flex-col items-center">
                        <span className="text-sm font-bold text-slate-700 truncate max-w-full">{detail.report.checkedBy || "-"}</span>
                        {detail.report.checkedBy && (
                          <div className="absolute right-0.5 bottom-0.5 text-sm border border-blue-200 text-blue-500 rounded px-0.5 rotate-12 bg-white/90 font-serif font-black scale-90">
                            검토
                          </div>
                        )}
                      </div>
                  ...
                  <div className="flex flex-col items-center justify-center p-2 bg-slate-50/70 rounded-xl border border-slate-100 text-center relative overflow-hidden">
                    <span className="text-sm font-bold text-slate-400">승인</span>
                    {isEditMode ? (
                      <Input
                        disabled={isReportLocked}
                      ...
                      <div className="mt-1 flex flex-col items-center">
                        <span className="text-sm font-bold text-slate-700 truncate max-w-full">{detail.report.approvedBy || "-"}</span>
                        {detail.report.approvedBy && (
                          <div className="absolute right-0.5 bottom-0.5 text-sm border border-emerald-200 text-emerald-500 rounded px-0.5 rotate-12 bg-white/90 font-serif font-black scale-90">
                            승인
                          </div>
                        )}
                      </div>
                  ...
                  <div className="flex flex-col items-center justify-center p-2 bg-slate-50/70 rounded-xl border border-slate-100 text-center">
                    ...
                    {isEditMode ? (
                      <select
                        ...
                        disabled={isReportLocked}
                        className="mt-1 h-6 rounded border border-slate-200 bg-white px-1 text-sm w-full"
                      >
                      ...
                      <span className="mt-1 text-sm font-semibold text-slate-700 truncate max-w-full">
            ...
            <Card className="border-slate-200/80 shadow-[0_8px_30px_rgb(0,0,0,0.02)] overflow-hidden">
              <CardHeader className="pb-3 bg-slate-50/30 border-b border-slate-100">
                <CardTitle className="flex items-center justify-between gap-3 text-sm font-bold text-slate-700">
                  ...
                  <Badge variant={operationalStats.readinessPercent >= 85 ? "default" : "secondary"} className="text-sm">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between bg-white dark:bg-slate-900 p-3 rounded-xl border border-slate-100">
                  <div className="space-y-0.5">
                    <div className="flex items-baseline gap-1">
                      ...
                      <span className="text-sm text-muted-foreground">정비도</span>
                    </div>
                    <p className="text-sm text-muted-foreground">조직도 연동, 역할 분장, 평가 매트릭스 입력 및 자격 매칭 기준</p>
                  </div>
          ...
          <Tabs defaultValue="organization" className="space-y-4">
            <TabsList className="grid w-full grid-cols-3 bg-slate-100/80 p-1 rounded-xl border border-slate-200/50">
              <TabsTrigger value="organization" className="rounded-lg font-bold text-sm">조직 및 업무분장</TabsTrigger>
              <TabsTrigger value="competency" className="rounded-lg font-bold text-sm">업무/요구기준 평가</TabsTrigger>
              <TabsTrigger value="qualifications" className="rounded-lg font-bold text-sm">요구자격 보유현황</TabsTrigger>
            </TabsList>

            {/* Tab 1: Organization & Responsibilities */}
            <TabsContent value="organization" className="space-y-4">
              <div className="flex items-center justify-between">
                <Badge variant="outline" className="bg-white/80 text-sm">{detail.report.teamName} / {detail.report.evaluationYear}</Badge>
                <div className="flex items-center gap-2">
                  <div className="flex rounded-lg border border-slate-200 bg-slate-100/80 p-0.5">
                    <button
                      type="button"
                      onClick={() => setOrgViewMode("tree")}
                      className={`flex items-center gap-1 px-2.5 py-1 text-sm font-bold rounded-md transition-all ${
            ...
            <TabsContent value="competency" className="space-y-4">
              <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between bg-slate-50/50 p-3.5 rounded-xl border border-slate-200/60">
                <div className="flex flex-wrap items-center gap-2 text-sm">
                  <Badge variant="outline" className="bg-white/80 text-sm">업무 영역 {detail.workCategories.length}개</Badge>
                  <Badge variant="outline" className="bg-white/80 text-sm">세부 요구사항 {detail.requirements.length}개</Badge>
                  <Badge variant="outline" className="bg-white/80 text-sm">매칭도 {scoreStats.entered}/{scoreStats.total}</Badge>
                  {operationalStats && operationalStats.missingScores > 0 && (
                    <Badge variant="secondary" className="bg-amber-50 text-amber-700 border-amber-200 text-sm">미평가 {operationalStats.missingScores}칸</Badge>
                  )}
                </div>

                <div className="flex flex-wrap items-center gap-2.5">
                  <div className="flex items-center gap-1.5 bg-white p-1 rounded-lg border border-slate-200">
                    <label className="text-sm font-bold text-muted-foreground px-1.5 uppercase">임직원 평가</label>
                    <select
                      value={selectedMember?.id ?? ""}
                      onChange={(event) => setSelectedMemberId(event.target.value)}
                      className="h-7 min-w-[130px] rounded-md border-0 bg-transparent px-1.5 text-sm font-semibold focus-visible:ring-0 cursor-pointer"
                  ...
                  <Button
                    ...
                    disabled={isReportLocked}
                    className="text-sm h-8 border-slate-200 hover:bg-slate-50 font-bold"
                  >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-5 gap-4">
                  <Card className="border-slate-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.01)] bg-slate-50/20">
                    <CardContent className="p-3">
                      <div className="text-sm font-bold text-slate-400 uppercase">대상 임직원</div>
                      <div className="mt-1 font-bold text-slate-800 text-sm flex items-center gap-1.5">
                        <Users className="h-4.5 w-4.5 text-indigo-500" />
                        {selectedMember.name}
                      </div>
                      <div className="text-sm text-slate-400 mt-0.5">{selectedMember.position}</div>
                    </CardContent>
                  </Card>

                  <Card className="border-slate-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.01)] bg-slate-50/20">
                    <CardContent className="p-3">
                      <div className="text-sm font-bold text-slate-400 uppercase">평균 요구 수준</div>
                      <div className="mt-1 text-base font-black text-slate-800">
                        {formatAverage(competencyStats.requiredAverage)} <span className="text-sm text-muted-foreground font-normal">/ 5</span>
                      </div>
                      <div className="text-sm text-indigo-600 font-semibold mt-0.5">{requirementGroupForMember(selectedMember).label} 요건</div>
                    </CardContent>
                  </Card>

                  <Card className="border-slate-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.01)] bg-slate-50/20">
                    <CardContent className="p-3">
                      <div className="text-sm font-bold text-slate-400 uppercase">현재 평가 수준</div>
                      <div className="mt-1 text-base font-black text-slate-800">
                        {formatAverage(competencyStats.selectedAverage)} <span className="text-sm text-muted-foreground font-normal">/ 5</span>
                      </div>
                      <div className="text-sm text-emerald-600 font-semibold mt-0.5">{selectedMember.name} 임직원 기준</div>
                    </CardContent>
                  </Card>

                  <Card className="border-slate-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.01)] bg-slate-50/20">
                    <CardContent className="p-3">
                      <div className="text-sm font-bold text-slate-400 uppercase">GAP 분석</div>
                      ...
                      <div className="text-sm text-slate-400 mt-0.5">요구대비 미달/초과 격차</div>
                    </CardContent>
                  </Card>

                  <Card className="border-slate-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.01)] bg-slate-50/20">
                    <CardContent className="p-3">
                      <div className="text-sm font-bold text-slate-400 uppercase">역량 보유율</div>
                      <div className="mt-1 text-base font-black text-slate-800">
                        {selectedLevelTotal > 0
                          ? Math.round((competencyStats.selectedScores.filter(s => s >= 3).length / selectedLevelTotal) * 100)
                          : 0}%
                      </div>
                      <div className="text-sm text-slate-400 mt-0.5">Lv.3 이상 확보 수준</div>
                    </CardContent>
                  </Card>
                </div>
              )}
              ...
              {selectedMember && selectedLevelTotal > 0 && (
                <Card className="border-slate-200/60 shadow-sm overflow-hidden">
                  <div className="p-3 bg-slate-50/30 border-b border-slate-100 flex items-center justify-between">
                    <div>
                      <span className="text-sm font-bold text-slate-700">개인 역량 분포 그래프</span>
                      <span className="text-sm text-slate-400 ml-1.5">{selectedMember.name}의 현재 직무수행 레벨 개수</span>
                    </div>
                    <Badge variant="outline" className="text-sm bg-white text-indigo-600 border-indigo-100">
                ...
                <fieldset disabled={isReportLocked} className="contents">
                  <div className="overflow-x-auto" data-testid="team-competency-merged-matrix">
                    <Table className="min-w-[1280px]">
                      <TableHeader>
                        <TableRow className="bg-slate-50 text-sm font-bold">
                          <TableHead className="w-[260px]">대분류 / 세부 업무</TableHead>
                          <TableHead className="min-w-[220px]">최소 실무지식 / 경력</TableHead>
                          <TableHead className="w-[150px] text-center">요구수준 / 팀평균</TableHead>
                          <TableHead className="w-[110px] text-center">해당 여부</TableHead>
                          <TableHead className="w-[270px] text-center">수행 역량 평가</TableHead>
                          <TableHead className="w-[90px] text-center">GAP</TableHead>
                          <TableHead className="w-[110px] text-center">육성 필요도</TableHead>
                          <TableHead className="min-w-[220px]">교육 및 어학 요건</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {continuousCompetencyGroups.length === 0 ? (
                          <TableRow>
                            <TableCell colSpan={8} className="py-12 text-center text-sm text-muted-foreground">
                              대분류와 세부업무가 아직 없습니다. 상단의 대분류 추가 버튼으로 업무 기준을 먼저 등록해 주세요.
                            </TableCell>
                          </TableRow>
                        ) : (
                          continuousCompetencyGroups.flatMap((group, groupIndex) => {
                            const category = group.workCategory;
                            const categoryIndex = group.workCategoryIndex;
                            const majorNo = category?.categoryNo ?? group.majorNo ?? String(groupIndex + 1);
                            const majorName = category?.categoryName ?? group.majorName ?? "대분류 정보 없음";
                            const completedRows = selectedMember
                              ? group.rows.filter(({ row }) => hasText(scoreFor(row, selectedMember.id))).length
                              : 0;
                            const groupHeader = (
                              <TableRow key={`${group.key}-header`} className="border-y-2 border-indigo-100 bg-indigo-50/70 hover:bg-indigo-50">
                                <TableCell colSpan={8} className="px-4 py-3">
                                  <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
                                    <div className="space-y-2">
                                      <div className="flex flex-wrap items-center gap-2">
                                        <span className="inline-flex min-w-8 items-center justify-center rounded-md bg-indigo-600 px-2 py-1 text-sm font-black text-white">
                                          {majorNo}
                                        </span>
                                        <span className="text-base font-black text-slate-900">{majorName}</span>
                                        <Badge variant="outline" className="bg-white text-sm font-semibold">
                                          세부업무 {group.rows.length}개
                                        </Badge>
                                        {selectedMember && (
                                          <Badge variant="secondary" className="bg-white text-sm font-semibold text-indigo-700">
                                            {completedRows}/{group.rows.length} 평가
                                          </Badge>
                                        )}
                                      </div>
                                      {(category?.majorFunctions || category?.controlItems) && (
                                        <div className="grid gap-3 text-sm leading-relaxed text-slate-700 md:grid-cols-2">
                                          {category?.majorFunctions && (
                                            <div className="rounded-lg border border-indigo-100 bg-white/80 px-3 py-2">
                                              <div className="mb-1 text-sm font-bold text-indigo-700">주요 업무 기능</div>
                                              <p className="whitespace-pre-wrap">{category.majorFunctions}</p>
                                            </div>
                                          )}
                                          {category?.controlItems && (
                                            <div className="rounded-lg border border-indigo-100 bg-white/80 px-3 py-2">
                                              <div className="mb-1 text-sm font-bold text-indigo-700">중점관리 항목 / Tool</div>
                                              <p className="whitespace-pre-wrap">{category.controlItems}</p>
                                            </div>
                                          )}
                                        </div>
                                      )}
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                      {category && categoryIndex !== undefined && (
                                        <Dialog>
                                          <DialogTrigger asChild>
                                            <Button type="button" variant="outline" size="sm" className="h-9 bg-white text-sm font-bold">
                                          ...
                                          <DialogContent className="max-w-xl">
                                            <DialogHeader>
                                              ...
                                              <DialogDescription>대분류 코드, 명칭, 주요 기능 및 중점관리 항목을 관리합니다.</DialogDescription>
                                            </DialogHeader>
                                            <div className="space-y-4 py-2 text-sm">
                                              <div className="grid grid-cols-4 gap-2">
                                                <div className="col-span-1">
                                                  <label className="text-sm font-bold text-slate-600">No</label>
                                                  <Input
                                                    disabled={isReportLocked}
                                                    value={category.categoryNo ?? ""}
                                                    onChange={(event) => updateArrayRow<WorkCategory>("workCategories", categoryIndex, { categoryNo: event.target.value })}
                                                    className="mt-1 text-sm"
                                                  />
                                                </div>
                                                <div className="col-span-3">
                                                  <label className="text-sm font-bold text-slate-600">대분류 명칭</label>
                                                  <Input
                                                    disabled={isReportLocked}
                                                    value={category.categoryName ?? ""}
                                                    onChange={(event) => updateArrayRow<WorkCategory>("workCategories", categoryIndex, { categoryName: event.target.value })}
                                                    className="mt-1 text-sm"
                                                  />
                                                </div>
                                              </div>
                                              <div>
                                                <label className="text-sm font-bold text-slate-600">주요 업무 기능</label>
                                                <Textarea
                                                  disabled={isReportLocked}
                                                  value={category.majorFunctions ?? ""}
                                                  onChange={(event) => updateArrayRow<WorkCategory>("workCategories", categoryIndex, { majorFunctions: event.target.value })}
                                                  rows={3}
                                                  className="mt-1 text-sm"
                                                />
                                              </div>
                                              <div>
                                                <label className="text-sm font-bold text-slate-600">중점관리 항목 / Tool</label>
                                                <Textarea
                                                  disabled={isReportLocked}
                                                  value={category.controlItems ?? ""}
                                                  onChange={(event) => updateArrayRow<WorkCategory>("workCategories", categoryIndex, { controlItems: event.target.value })}
                                                  rows={3}
                                                  className="mt-1 text-sm"
                                                />
                                              </div>
                                      ...
                                      <Button
                                        ...
                                        variant="outline"
                                        size="sm"
                                        onClick={() => addRequirementForCategory(majorNo, majorName, group.rows.length + 1)}
                                        disabled={isReportLocked}
                                      ...
                                      {isEditMode && categoryIndex !== undefined && (
                                        <Button
                                          type="button"
                                          variant="ghost"
                                          size="sm"
                                          onClick={() => removeArrayRow("workCategories", categoryIndex)}
                                          disabled={isReportLocked}
                                          className="h-9 bg-white text-sm font-bold text-slate-500 hover:text-destructive"
                                        >
                            ...
                            if (group.rows.length === 0) {
                              return [
                                ...
                                <TableRow key={`${group.key}-empty`}>
                                  <TableCell colSpan={8} className="bg-white py-8 text-center text-sm text-muted-foreground">
                                    이 대분류 영역에 아직 세부업무가 없습니다. 우측의 세부업무 추가 버튼을 눌러 첫 항목을 작성해 주세요.
                                  </TableCell>
                                </TableRow>,
                              ];
                            }
                            ...
                            return [
                              ...
                              ...group.rows.map(({ row, index }) => {
                                ...
                                return (
                                  <TableRow key={`${group.key}-${row.id ?? index}`} className={`text-sm hover:bg-slate-50/40 ${gap !== null && gap < 0 ? "bg-rose-50/10" : ""}`}>
                                    <TableCell className="align-top font-medium">
                                      {isEditMode ? (
                                        <div className="space-y-2">
                                          <div className="flex items-start gap-2">
                                            <TextCell value={row.subNo} onChange={(value) => updateArrayRow<Requirement>("requirements", index, { subNo: value })} placeholder="No" className="h-9 w-14 text-center text-sm" />
                                            <TextCell value={row.subName} onChange={(value) => updateArrayRow<Requirement>("requirements", index, { subName: value })} placeholder="세부업무명" className="h-9 w-full text-sm" />
                                            <Button
                                              variant="ghost"
                                              size="sm"
                                          ...
                                          <div className="grid grid-cols-2 gap-2">
                                            <TextCell value={row.requiredMajor} onChange={(value) => updateArrayRow<Requirement>("requirements", index, { requiredMajor: value })} placeholder="권장 전공" className="h-8 w-full text-sm" />
                                            <TextCell value={row.requiredCertification} onChange={(value) => updateArrayRow<Requirement>("requirements", index, { requiredCertification: value })} placeholder="권장 자격증" className="h-8 w-full text-sm" />
                                          </div>
                                        </div>
                                      ) : (
                                        <div className="space-y-2">
                                          <div className="flex items-start gap-2">
                                            <span className="mt-0.5 shrink-0 rounded bg-slate-100 px-2 py-0.5 text-sm font-black text-slate-600">
                                              {row.subNo || "-"}
                                            </span>
                                            <span className="font-bold text-slate-900">{row.subName || "업무명 미입력"}</span>
                                          </div>
                                          <div className="flex flex-wrap gap-1.5 pl-8">
                                            {row.requiredMajor && (
                                              <Badge variant="outline" className="bg-slate-50 text-sm font-bold text-slate-600">
                                                {row.requiredMajor}
                                              </Badge>
                                            )}
                                            {row.requiredCertification && (
                                              <Badge variant="secondary" className="bg-indigo-50 text-sm font-bold text-indigo-700">
                                                {row.requiredCertification}
                                              </Badge>
                                            )}
                                          </div>
                                      ...
                                      {isEditMode ? (
                                        <div className="space-y-2">
                                          <TextAreaCell
                                            ...
                                            className="min-h-[64px] w-full text-sm"
                                          />
                                          <TextCell
                                        ...
                                        <div className="space-y-1.5">
                                          <p className="whitespace-pre-wrap text-sm font-medium leading-relaxed text-slate-700">{row.minKnowledge || "-"}</p>
                                          {row.proficiencyPeriod && (
                                            <p className="text-sm font-semibold text-slate-500">숙달소요: {row.proficiencyPeriod}</p>
                                          )}
                                        </div>
                                      )}
                                    </TableCell>
                                    <TableCell className="text-center align-top">
                                      <div className="flex flex-col items-center gap-1.5">
                                        <span className={`inline-flex rounded-md border px-2 py-1 text-sm font-bold ${levelBadgeClass(requiredLevel)}`}>
                                          요구 {formatLevel(selectedRequirementValue)}
                                        </span>
                                        <span className="text-sm font-medium text-slate-500">{selectedRequirementGroup.label}</span>
                                        <span className={`inline-flex rounded-md border px-2 py-1 text-sm font-bold ${levelBadgeClass(teamAverage, requiredLevel)}`}>
                                          평균 {formatLevel(teamAverage)}
                                        </span>
                                        <Dialog>
                                          <DialogTrigger asChild>
                                            <Button variant="ghost" size="sm" className="h-8 w-full border border-dashed border-slate-200 text-sm font-bold text-slate-600 hover:bg-slate-50">
                                              기준 변경
                                            </Button>
                                          </DialogTrigger>
                                          <DialogContent className="max-w-md">
                                            ...
                                            <div className="grid gap-3 py-2">
                                              <div className="grid grid-cols-3 items-center gap-3">
                                                <label className="text-sm font-bold text-slate-600">책임 미만</label>
                                                <TextCell value={row.staffLevel} onChange={(value) => updateArrayRow<Requirement>("requirements", index, { staffLevel: value })} placeholder="Lv 또는 N/A" disabled={isReportLocked} className="col-span-2" />
                                              </div>
                                              <div className="grid grid-cols-3 items-center gap-3">
                                                <label className="text-sm font-bold text-slate-600">책임 이상</label>
                                                <TextCell value={row.managerLevel} onChange={(value) => updateArrayRow<Requirement>("requirements", index, { managerLevel: value })} placeholder="Lv 또는 N/A" disabled={isReportLocked} className="col-span-2" />
                                              </div>
                                              <div className="grid grid-cols-3 items-center gap-3">
                                                <label className="text-sm font-bold text-slate-600">팀장·그룹장</label>
                                                <TextCell value={row.deptHeadLevel} onChange={(value) => updateArrayRow<Requirement>("requirements", index, { deptHeadLevel: value })} placeholder="Lv 또는 N/A" disabled={isReportLocked} className="col-span-2" />
                                              </div>
                                              <div className="grid grid-cols-3 items-center gap-3 rounded-lg border border-yellow-100 bg-yellow-50/50 p-2">
                                                <label className="text-sm font-bold text-yellow-800">최소 한계수준</label>
                                                <TextCell value={row.minimumLevel} onChange={(value) => updateArrayRow<Requirement>("requirements", index, { minimumLevel: value })} placeholder="Lv" disabled={isReportLocked} className="col-span-2 bg-white" />
                                              </div>
                                            </div>
                                          </DialogContent>
                                        </Dialog>
                                    ...
                                    <TableCell className="text-center align-top">
                                      {selectedMember ? (
                                        <div className="flex flex-col items-center justify-center gap-1.5 pt-2">
                                          ...
                                          />
                                          <span className={`text-sm font-bold ${selectedApplicable ? "text-indigo-700" : "text-slate-400"}`}>
                                    ...
                                    <TableCell className="text-center align-top">
                                      {selectedMember ? (
                                        <div className="space-y-2 py-1">
                                          ...
                                          />
                                          <div className="flex items-center justify-between px-4 text-sm font-bold text-muted-foreground">
                                            <span>미흡</span>
                                            <span>보통</span>
                                            <span>우수</span>
                                          </div>
                                    ...
                                    <TableCell className="text-center align-top">
                                      <div className="pt-2">
                                        <span className={`inline-flex rounded-md border px-2 py-1 text-sm font-bold ${
                                          gap !== null && gap < 0
                                    ...
                                    <TableCell className="text-center align-top">
                                      <div className="pt-2">
                                        <span className={`inline-flex rounded-md border px-2 py-1 text-sm font-bold ${need.className}`}>
                                          {need.label}
                                        </span>
                                      </div>
                                    </TableCell>
                                    <TableCell className="align-top">
                                      {isEditMode ? (
                                        ...
                                        <div className="space-y-2 text-sm">
                                          <p className="font-semibold leading-relaxed text-slate-700">{row.requiredTraining || "-"}</p>
                                          {toolHint && (
                                            <p className="inline-block rounded border border-indigo-100/60 bg-indigo-50/40 px-2 py-1 text-sm font-semibold text-indigo-700">
                                              중점: {toolHint}
                                            </p>
                                          )}
                                          {row.languageRequirement && (
                                            <p className="text-sm font-bold text-slate-500">어학: {row.languageRequirement}</p>
                                          )}
                                        </div>
                                      )}
                                    </TableCell>
            ...
            <TabsContent value="qualifications" className="space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-50/50 p-3 rounded-xl border border-slate-200/60">
                <div className="flex flex-wrap gap-2 text-sm font-semibold">
                  <Badge variant="outline" className="bg-white/80 text-slate-600 text-sm">자격 {evidenceStats.certifications}건</Badge>
                  <Badge variant="outline" className="bg-white/80 text-slate-600 text-sm">스킬 {evidenceStats.skills}건</Badge>
                  <Badge variant="outline" className="bg-white/80 text-slate-600 text-sm">어학 {evidenceStats.languages}건</Badge>
                  <Badge variant="outline" className="bg-white/80 text-slate-600 text-sm">사외교육 {evidenceStats.trainings}건</Badge>
                  {operationalStats && operationalStats.missingQualificationHolders > 0 && (
                    <Badge variant="secondary" className="bg-amber-50 text-amber-700 border-amber-200 text-sm">보유자 미입력 {operationalStats.missingQualificationHolders}건</Badge>
                  )}
                </div>
                ...
                <Button
                  ...
                  disabled={isReportLocked}
                  className="text-sm font-bold border-slate-200 h-8 hover:bg-slate-50"
                >
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {detail.qualifications.length === 0 ? (
                          <TableRow>
                            <TableCell colSpan={isEditMode ? 7 : 6} className="text-center py-10 text-sm text-muted-foreground">
                          ...
                          detail.qualifications.map((row, index) => (
                            <TableRow key={row.id ?? index} className="text-sm hover:bg-slate-50/30">
                              {/* No */}
                              <TableCell className="align-top font-bold text-slate-500">
                                {isEditMode ? (
                                  <TextCell value={row.itemNo} onChange={(value) => updateArrayRow<Qualification>("qualifications", index, { itemNo: value })} className="w-16 h-8 text-center text-sm" />
                                ) : (
                              ...
                              <TableCell className="align-top font-bold text-slate-700">
                                {isEditMode ? (
                                  <TextCell value={row.requirementItem} onChange={(value) => updateArrayRow<Qualification>("qualifications", index, { requirementItem: value })} className="h-8 text-sm w-full" />
                                ) : (
                              ...
                              <TableCell className="align-top">
                                {isEditMode ? (
                                  <TextAreaCell value={row.requirementName} onChange={(value) => updateArrayRow<Qualification>("qualifications", index, { requirementName: value })} className="min-h-[40px] text-sm w-full" />
                                ) : (
                              ...
                              <TableCell className="align-top">
                                {isEditMode ? (
                                  <TextCell value={row.requiredGrade} onChange={(value) => updateArrayRow<Qualification>("qualifications", index, { requiredGrade: value })} className="h-8 text-sm w-full" />
                                ) : (
                                  <span className="mt-1 inline-block bg-indigo-50 border border-indigo-100 text-indigo-700 rounded px-2 py-0.5 font-bold text-sm">
                                    {row.requiredGrade || "-"}
                                  </span>
                                )}
                              </TableCell>
                              ...
                              <TableCell className="align-top">
                                {isEditMode ? (
                                  <div className="space-y-2">
                                    <TextAreaCell value={row.holderSummary} onChange={(value) => updateArrayRow<Qualification>("qualifications", index, { holderSummary: value })} placeholder="보유자 명단 기재" className="min-h-[40px] text-sm w-full" />
                                    <TextCell value={row.heldQualification} onChange={(value) => updateArrayRow<Qualification>("qualifications", index, { heldQualification: value })} placeholder="취득 자격등급" className="h-8 text-sm w-full" />
                                  </div>
                                ) : (
                                    ...
                                    {row.holderSummary ? (
                                      <div className="flex flex-wrap gap-1">
                                        {row.holderSummary.split(/[\s,]+/).filter(Boolean).map((name, nIdx) => (
                                          <Badge key={nIdx} variant="secondary" className="bg-emerald-50 border-emerald-100 text-emerald-700 text-sm font-bold">
                                            {name}
                                          </Badge>
                                        ))}
                                      </div>
                                    ) : (
                                      <Badge variant="outline" className="bg-rose-50 border-rose-100 text-rose-600 text-sm font-bold">
                                        보유자 공란
                                      </Badge>
                                    )}
                                    {row.heldQualification && (
                                      <p className="text-sm text-slate-400 font-medium italic">자격내용: {row.heldQualification}</p>
                                    )}
                                  </div>
                                )}
                              </TableCell>
                              ...
                              <TableCell className="align-top">
                                {isEditMode ? (
                                  <div className="space-y-2">
                                    <TextAreaCell value={row.plan} onChange={(value) => updateArrayRow<Qualification>("qualifications", index, { plan: value })} placeholder="향후 확보 계획" className="min-h-[40px] text-sm w-full" />
                                    <TextAreaCell value={row.remarks} onChange={(value) => updateArrayRow<Qualification>("qualifications", index, { remarks: value })} placeholder="비고" className="min-h-[40px] text-sm w-full" />
                                  </div>
                                ) : (
                                  <div className="space-y-1 mt-1 text-sm leading-relaxed">
                                    {row.plan && (
                                      <p className="text-slate-600"><span className="font-bold text-slate-400 text-sm mr-1">[계획]</span>{row.plan}</p>
                                    )}
                                    {row.remarks && (
                                      <p className="text-slate-400"><span className="font-bold text-slate-400 text-sm mr-1">[비고]</span>{row.remarks}</p>
                                    )}
        ...
        <div className="flex min-h-[420px] items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50/15">
          <div className="max-w-md text-center p-6 space-y-4">
            ...
            <Button className="font-bold text-sm bg-indigo-600 hover:bg-indigo-700" onClick={() => ensureReportMutation.mutate()} disabled={!selectedTeam || isLoadingDetail}>
              <Plus className="mr-1.5 h-4 w-4" />
              보고서 준비
            </Button>
          </div>
